...
import { modelOptionLabel, modelOptionName, normalizeModelOptionValue, resolveModelChannel, selectableModelsByCapability, useConfigStore, useEffectiveConfig, type AiConfig } from "@/stores/use-config-store";
...
function AgentTextModelPicker({ config, value, onChange }: { config: AiConfig; value: string; onChange: (model: string) => void }) {
    ...
    const current = value || "";
    const showChannel = config.channels.length > 1;
    return (
        <Select value={current} onValueChange={onChange}>
            ...
            >
                ...
                {current && showChannel ? <span className="shrink-0 opacity-55">{resolveModelChannel(config, current).name}</span> : null}
            </SelectTrigger>
                    ...
                    options.map((model) => (
                        <SelectItem key={model} value={model} textValue={modelOptionLabel(config, model)}>
                            <span className="flex min-w-0 items-center gap-2">
                                ...
                                {showChannel ? <span className="shrink-0 text-xs opacity-55">{resolveModelChannel(config, model).name}</span> : null}
                            </span>
                        </SelectItem>
                    ))
                ) : (
